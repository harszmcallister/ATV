...
export interface Step {
  ...
  text_overlay: string;
  [key: string]: any; // Allows additional JSON manifest properties
}
...
export const MicroSceneComp: React.FC<MicroSceneProps> = ({ step, comparison, language }) => {
  ...
  return (
    <div style={{ 
      ...
      {/* 1. TOP PORTION: Side-by-Side Item Display */}
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start', width: '100%', height: '580px', marginTop: '140px', gap: '30px' }}>
        ...
        <div style={{ width: '460px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          {step.visible_animals && step.visible_animals[0] && (
            <>
              <div style={{ fontSize: '48px', fontWeight: 'bold', color: '#111', marginBottom: '24px', textAlign: 'center', whiteSpace: 'nowrap' }}>
                {titleA}
              </div>
              <img
        ...
        {/* Right Slot: Item 2 */}
        <div style={{ width: '460px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          {step.visible_animals && step.visible_animals[1] && (
            <>
      ...
      {step.selected_stickman_pose && (
        <div style={{ position: 'absolute', bottom: '140px', left: '0', right: '0', textAlign: 'center' }}>
          <img
